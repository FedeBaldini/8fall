(async () => {
  ...
  switch (options.update) {
    ...
    case "presale_whitelisted_addresses":
      const addresses_add = PRESALE_WHITELISTED_ADDRESSES;
    ...
    case "presale_whitelisted_addresses_remove":
      const addresses_remove = PRESALE_WHITELISTED_ADDRESSES;
      const existingAddressesList = await getWhiteList();
      const updatedAddresses = existingAddressesList.filter(
        (address) => !addresses_remove.includes(address)
      );
      contract.presale_whitelisted_addresses = updatedAddresses;
      updateValue = `remove ${PRESALE_WHITELISTED_ADDRESSES}`;
      break;
    case "presale_whitelisted_addresses_empty":
      contract.presale_whitelisted_addresses = [];
      updateValue = `empty the whitelist`;
      break;
      ò;
    case "royalty_share":
